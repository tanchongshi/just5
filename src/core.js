...
just5.fn = just5.prototype = {
	...
	init: function(params, context) {
		...
		if(context){
			if(context.nodeType){//document 元素 或者 Element 元素
				currentContext = context;
			}else{//如果是字符 ，基于document选择该选择器
		...
		if(typeof params === 'string' && /<(\S*?)[^>]*>.*?<\/\1>/.test(params)){
			...
			return this; //return 实例. {0:ELEMENT_NODE,1:ELEMENT_NODE,length:2}
		}
...
//扩展类方法，清空前后空格
just5.trim = String.prototype.trim?function(a){return a.trim();}:function(a){return a.replace(/^[\s\xa0]+|[\s\xa0]+$/g,'');};
